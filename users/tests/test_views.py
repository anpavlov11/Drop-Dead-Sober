...
class TestSignUpView(TestCase):
    ...
    def test_new_user_is_registered(self):
        # Check that a user has been registered with the method with count()
        nb_old_users = User.objects.count()  # count users before a request
        self.client.post(reverse('users:signup'), self.data)
        nb_new_users = User.objects.count()  # count users after, make sure 1 user was added
        self.assertEqual(nb_new_users, nb_old_users + 1)

    def test_redirect_if_user_is_authenticated(self):
        # If the user is authenticated and try to access the signup page..user is redirected to the home page
